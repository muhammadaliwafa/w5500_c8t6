...
void parse_http_request(
    ...
    char * nexttok;
    uint8_t cp =0;
//    printf("parse_http_request: awal %s \n", buf);
    ...
    if (!strcmp(nexttok, "GET") || !strcmp(nexttok, "get")) {
		...
		size_t len = strnlen(nexttok, MAX_URI_SIZE);
		memcpy((char *)request->URI, nexttok, len);
//        int len = strnlen(nexttok, MAX_URI_SIZE);
//        if(len<=MAX_URI_SIZE)
//        *(((uint8_t *)nexttok) + (int)len+1) = 0x00;




//        printf("\n>>>>>parse_http_request: ditemukan %d%s\n", len);
//        if (strnlen(nexttok, MAX_URI_SIZE) == MAX_URI_SIZE) {
//        	nexttok[MAX_URI_SIZE - 1] = '\0';  // Force null terminator
//        }

//		for(int i=0; i<MAX_URI_SIZE; i++){// ini dilakukan karena sebelumnya terjadi block waktu copy data di bawah
//			if(nexttok[i]=='\0'){
//				*(((uint8_t *)nexttok) + i+1) = 0x00;
////				*(((uint8_t *)nexttok) + i+2) = 0x00;
////				printf("parse_http_request: POST %s\n", nexttok);
////				printf("\n>>>>>parse_http_request: ditemukan %d%s\n", i);
//				break;
//			}
//		}
//
//        printf("\n>>>>>> data aneh: \n");
//        HAL_UART_Transmit(&huart1, &nexttok[1017], 10, 200);
//        printf("\n");
//        *((uint8_t *)nexttok) = 0x00;
...
//    size_t len = strnlen(nexttok, MAX_URI_SIZE)+1;
//    memcpy((char *)request->URI, nexttok, len);
    if(!cp)
    strcpy((char *)request->URI, nexttok);
//    printf("parse_http_request: METHOD %s\n", request->URI);
//    printf("parse_http_request: %s\n", nexttok);
