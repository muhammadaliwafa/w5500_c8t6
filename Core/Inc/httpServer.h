...
typedef struct _httpServer_webContent_flash {
    uint8_t	*	content_name;
    uint32_t	addr;
} httpServer_flashContent;
...
void reg_httpServer_flashContent(uint8_t * content_name, uint32_t addr);//ALI
uint8_t find_userReg_webContent(uint8_t * content_name, uint16_t * content_num, uint32_t * file_len);
uint8_t find_userReg_flashContent(uint8_t * content_name, uint32_t * addr);// ALI
